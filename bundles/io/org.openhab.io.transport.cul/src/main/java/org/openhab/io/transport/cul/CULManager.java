...
/**
 * This class handles all CULHandler. You can only obtain CULHandlers via this manager.
 * 
 * @author Till Klocke
 * @since 1.4.0
 */
public class CULManager {
    ...
    /**
     * Get CULHandler for the given device in the given mode. The same CULHandler can be returned multiple times if you ask multiple times for the same device
     * in the same mode. It is not possible to obtain a CULHandler of an already openend device for another RF mode.
     * 
     * @param deviceName
     *            String representing the device. Currently only serial ports are supported.
     * @param mode
    ...
    /**
     * Return a CULHandler to the manager. The CULHandler will only be closed if there aren't any listeners registered with it. So it is save to call this
		...
		synchronized (openDevices) {
			...
			if (handler instanceof CULHandlerInternal) {
				...
				if (!internalHandler.hasListeners()) {
				    for (Entry<String, CULHandler> entry : openDevices.entrySet()) {
					if(handler.equals(entry.getValue())){
					    openDevices.remove(entry.getKey());
					}
