...
angular.module('windowEventBroadcasts', []).run([ '$rootScope', '$window', '$document', function($rootScope, $window, $document) {
	var window = angular.element($window), vendorPrefix, prevEvent;

	// We'll let jQuery/jqLite handle cross-browser compatibility with window
	// blur/focus
	// Blur events can be double-fired, so we'll filter those out with prevEvent
	// tracking
	...
	// With document visibility, we'll have to handle cross-browser
	// compatibility ourselves
	// Compatibility: IE10+, FF10+, Chrome 14+, Safari 6.1+, Opera 12.1+, iOS
	// Safari 7+
	// For more detailed compatibility statistics:
	...
	// Determine if a vendor prefix is required to utilize the Page Visibility
	// API
