...
public class ChartDataServlet extends WebSocketServlet {
	...
	public void stateChanged(final Item item, final State oldState, final State newState) {
	    ...
	    sendStateMessage(name, label, state);
	}

	private void sendStateMessage(String name, String label, State state) {
	    final JsonObject msg = createStateMessage(name, label, state, null);
	    for (final WebSocketImpl socket : this.sockets) {
		try {
    ...
    private static JsonObject createStateMessage(String name, String label, State state, Date timestamp) {
