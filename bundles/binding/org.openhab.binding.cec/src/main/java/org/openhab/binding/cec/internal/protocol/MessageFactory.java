...
public class MessageFactory {
	...
	public Message parse(Integer opCode, String payload) {
		MessageType definition = this.database.getMessage(opCode);

		if (definition == null) {
			throw new IllegalArgumentException("No MessageType defined for opCode 0x" + Integer.toString(opCode, 16));
		}
			...
			try {
				...
				Collections.sort(methodCandidates, new Comparator<Method>() {
					...
					public int compare(Method o1, Method o2) {
						if (int.class.equals(o1.getParameters()[0].getType())) {
							return -1;
						} else if (String.class.equals(o1.getParameters()[0].getType())) {
							return 1;
						}
						return 0;
					}
				...
				if (int.class.isAssignableFrom(method.getParameters()[0].getType())) {
