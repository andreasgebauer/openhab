...
public class UpdateFHTTimeJob implements Job {
	...
	private final static Logger log = LoggerFactory
			.getLogger(UpdateFHTTimeJob.class);
	...
	public void execute(JobExecutionContext context)
		...
		Iterator<FHTBindingConfig> bindingIterator = CULBinding.INSTANCE
				.getAllFHTBindingConfigs().iterator();
		while (bindingIterator.hasNext()) {
			FHTBindingConfig config = bindingIterator.next();
			log.debug("Updating time of FHT with address "
					+ config.getAddress());
			...
			if (bindingIterator.hasNext()) {
				try {
					...
					log.warn("Thread wouldn't sleep for the update interval of "
